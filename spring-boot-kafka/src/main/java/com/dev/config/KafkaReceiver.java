...
public class KafkaReceiver {
    ...
    /**
     * batchListener  and batchListener1 use one groupId
     *
     * @param messages
     */
     ...
     * @param messages
     */
    @KafkaListener(id = "batchListener1",
            topics = {TopicConst.EXECUTOR_TOPIC},
            containerFactory = "batchContainerFactory",
            groupId = TopicConst.EXECUTOR_GROUPID)
    public void batchListener1(List<String> messages) { //MAX_POLL_RECORDS_CONFIG
        long timenow = System.currentTimeMillis();
        try {
            Thread.sleep(new Random().nextInt(100) + 50);
        } catch (InterruptedException e) {
